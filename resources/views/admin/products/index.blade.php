                ...
                <form method="POST" action="{{ route('admin.products.store') }}" class="grid gap-5 border-t border-zinc-200 p-5 dark:border-zinc-800 lg:grid-cols-2">
                    ...
                    <div class="grid content-start gap-4 rounded-2xl bg-violet-50/60 p-5 dark:bg-violet-500/5">
                        ...
                        <div class="grid gap-4 sm:grid-cols-2">
                            ...
                            <label class="grid gap-1.5 text-sm font-medium">Pagamento<select name="billing_type" class="h-10 rounded-xl border border-zinc-300 bg-white px-3 font-normal dark:border-zinc-700 dark:bg-zinc-950">@foreach (App\Models\ProductPlan::BILLING_TYPES as $value => $label)<option value="{{ $value }}" @selected(old('billing_type', 'UNDEFINED') === $value)>{{ $label }}</option>@endforeach</select></label>
