<x-layouts::app :title="__('Clientes')">
    ...
    <div class="mx-auto flex w-full max-w-7xl flex-col gap-8">
        ...
        <section id="novo-cliente" class="portal-card scroll-mt-6 p-5 sm:p-6">
            <div class="mb-6"><h2 class="text-lg font-semibold">Cadastrar novo cliente</h2><p class="mt-1 text-sm text-zinc-500">Crie o acesso em poucos segundos. O cliente receberá um convite seguro para definir a própria senha.</p></div>
            <form method="POST" action="{{ route('admin.customers.store') }}" class="grid gap-5 md:grid-cols-2">
                ...
                <label class="grid gap-1.5 text-sm font-medium md:col-span-2">E-mail de acesso<input type="email" name="email" value="{{ old('email') }}" required class="h-11 rounded-xl border border-zinc-300 bg-white px-3 font-normal outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20 dark:border-zinc-700 dark:bg-zinc-900" />@error('email')<span class="text-xs text-red-600">{{ $message }}</span>@enderror</label>

                <details class="group rounded-xl border border-zinc-200 md:col-span-2 dark:border-zinc-800" @if(old('tax_id') || old('cellphone') || old('customer_group_id')) open @endif>
                    <summary class="flex cursor-pointer list-none items-center justify-between px-4 py-3 text-sm font-semibold"><span>Dados financeiros <span class="font-normal text-zinc-400">(opcional agora)</span></span><flux:icon.chevron-down class="size-4 transition group-open:rotate-180" /></summary>
                    <div class="grid gap-5 border-t border-zinc-200 p-4 md:grid-cols-2 dark:border-zinc-800">
                        <label class="grid gap-1.5 text-sm font-medium">CPF ou CNPJ<input name="tax_id" value="{{ old('tax_id') }}" placeholder="Pode ser preenchido depois" class="h-11 rounded-xl border border-zinc-300 bg-white px-3 font-normal outline-none focus:border-violet-500 dark:border-zinc-700 dark:bg-zinc-900" />@error('tax_id')<span class="text-xs text-red-600">{{ $message }}</span>@enderror</label>
                        <label class="grid gap-1.5 text-sm font-medium">Celular<input name="cellphone" value="{{ old('cellphone') }}" placeholder="(11) 99999-9999" class="h-11 rounded-xl border border-zinc-300 bg-white px-3 font-normal outline-none focus:border-violet-500 dark:border-zinc-700 dark:bg-zinc-900" />@error('cellphone')<span class="text-xs text-red-600">{{ $message }}</span>@enderror</label>
                        <label class="grid gap-1.5 text-sm font-medium md:col-span-2">Grupo<select name="customer_group_id" class="h-11 rounded-xl border border-zinc-300 bg-white px-3 font-normal outline-none focus:border-violet-500 dark:border-zinc-700 dark:bg-zinc-900"><option value="">Sem grupo</option>@foreach ($groups->where('active', true) as $group)<option value="{{ $group->id }}" @selected((int) old('customer_group_id') === $group->id)>{{ $group->name }}</option>@endforeach</select></label>
                    </div>
                </details>

                <div class="flex flex-col gap-2 md:col-span-2 md:flex-row md:items-center md:justify-between"><p class="text-xs text-zinc-500">CPF/CNPJ será obrigatório somente antes da primeira cobrança.</p><button class="inline-flex h-11 w-full items-center justify-center rounded-xl bg-violet-600 px-5 text-sm font-semibold text-white shadow-sm transition hover:bg-violet-500 md:w-auto">Criar e enviar convite</button></div>
            </form>
        </section>
