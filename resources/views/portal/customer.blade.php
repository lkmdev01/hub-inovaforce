    ...
    <div class="mx-auto flex w-full max-w-4xl flex-col gap-7">
        ...
        <div class="portal-card overflow-hidden">
            ...
            <form method="POST" action="{{ route('customer.update') }}" class="grid gap-5 p-6 sm:grid-cols-2">
                ...
                <label class="grid gap-1.5 text-sm sm:col-span-2"><span class="font-medium">Celular <span class="font-normal text-zinc-400">(opcional, recomendado para avisos)</span></span><input name="cellphone" value="{{ old('cellphone', $customer->cellphone) }}" placeholder="(11) 99999-9999" class="rounded-xl border-zinc-300 bg-white dark:border-zinc-700 dark:bg-zinc-900">@error('cellphone')<span class="text-xs text-red-600">{{ $message }}</span>@enderror</label>

                <details class="group rounded-xl border border-zinc-200 sm:col-span-2 dark:border-zinc-800" @if(old('zip_code') || old('address') || old('municipal_inscription') || $customer->zip_code || $customer->address) open @endif>
                    <summary class="flex cursor-pointer list-none items-center justify-between px-4 py-3 text-sm font-semibold"><span>Endereço e dados fiscais <span class="font-normal text-zinc-400">(preencha quando precisar emitir nota)</span></span><flux:icon.chevron-down class="size-4 transition group-open:rotate-180" /></summary>
                    <div class="grid gap-5 border-t border-zinc-200 p-4 sm:grid-cols-2 dark:border-zinc-800">
